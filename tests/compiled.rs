...
fn c_snapshot_round_trip() {
    ...
    let (usandbox, _cfg) =
        create_sandbox(&Some(rootfs), &None, 64, mounts_save, None, None).unwrap();
    let mut sandbox = init(usandbox).unwrap();
...
fn go_snapshot_round_trip() {
    ...
    let mounts_save = vec![Mount::rw(&empty_mount, BIN_MOUNT)];
    let (usandbox, _cfg) =
        create_sandbox(&Some(rootfs), &None, 128, mounts_save, None, None).unwrap();
    let mut sandbox = init(usandbox).unwrap();
    let snap = sandbox.snapshot().unwrap();
    let tag: OciTag = SNAPSHOT_TAG.parse().unwrap();
    snap.save(&snap_dir, &tag).unwrap();
...
fn dotnet_aot_hello() {
    ...
    let build_dir =
        std::env::temp_dir().join(format!("hluk-test-dotnet-aot-{}", std::process::id()));
...
fn dotnet_aot_snapshot_round_trip() {
    ...
    let empty_mount = std::env::temp_dir().join(format!(
        "hluk-test-dotnet-aot-snap-mount-{}",
        std::process::id()
    ));
    std::fs::create_dir_all(&empty_mount).unwrap();
    let mounts_save = vec![Mount::rw(&empty_mount, BIN_MOUNT)];
    let (usandbox, _cfg) =
        create_sandbox(&Some(rootfs), &None, 256, mounts_save, None, None).unwrap();
    let mut sandbox = init(usandbox).unwrap();
    let snap = sandbox.snapshot().unwrap();
    let tag: OciTag = SNAPSHOT_TAG.parse().unwrap();
    snap.save(&snap_dir, &tag).unwrap();
...
fn dotnet_aot_env_vars() {
    ...
    let build_dir =
        std::env::temp_dir().join(format!("hluk-test-dotnet-aot-env-{}", std::process::id()));
