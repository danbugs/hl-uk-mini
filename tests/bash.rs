...
fn bash_stdin_piped() {
    let rootfs = require_rootfs("bash");
    let script = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("examples/bash/stdin_echo.sh");
...
fn bash_shell_interactive() {
    let rootfs = require_rootfs("bash");
    let script = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("examples/bash/shell.sh");
    let output = hluk_with_stdin(&rootfs, &script, b"echo hello-from-shell\necho 42\n");
    assert!(output.contains("hello-from-shell"), "expected echo output, got: {output:?}");
    assert!(output.contains("42"), "expected second echo, got: {output:?}");
}
